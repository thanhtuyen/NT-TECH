...
class ControllerModulePavverticalmenu extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')  && !empty($this->request->post) ) {
			if (!$this->user->hasPermission('modify', 'module/'.$this->moduleName)) {
				...
				if( isset($id) && $this->request->post['save_mode']=='save-edit'){
					$this->redirect($this->url->link('module/pavverticalmenu', 'id='.$id.'&token=' . $this->session->data['token'], 'SSL'));
				}	else {
		...
		$this->data['positions'] = array( 	  'called_framework',
										  'mainmenu',
										  'slideshow',
										  'promotion',
										  'content_top',
		...
		$this->data['action'] = $this->url->link('module/pavverticalmenu', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['actionGetTree'] = $this->url->link('module/pavverticalmenu/gettree', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['actionDel'] = $this->url->link('module/pavverticalmenu/delete', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['actionGetInfo'] = $this->url->link('module/pavverticalmenu/info', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['updateTree'] = $this->url->link('module/pavverticalmenu/update', 'root=1&token=' . $this->session->data['token'], 'SSL');
		
		$this->data['liveedit_url'] = $this->url->link('module/pavverticalmenu/liveedit', 'root=1&token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['tree'] = $this->model_verticalmenu_verticalmenu->getTree( );
	...
	public function delete(){
		...
		if( isset($this->request->get['id']) ){
			$this->load->model('verticalmenu/verticalmenu');
			$this->model_verticalmenu_verticalmenu->delete( (int)$this->request->get['id'] );
	...
	public function info(){
		...
		$this->data['menu'] = $menu;  
		$this->data['menus'] = $this->model_verticalmenu_verticalmenu->getDropdown(null, $menu['parent_id'] );
	...
	public function liveedit(){
		...
		$this->data['liveedit_action'] = $this->url->link('module/pavverticalmenu/livesave', 'root=1&token=' . $this->session->data['token'], 'SSL');
		$this->data['action_backlink'] = $this->url->link('module/pavverticalmenu', 'root=1&token=' . $this->session->data['token'], 'SSL');
		$this->data['action_widget'] =  HTTPS_CATALOG.'index.php?route=module/pavverticalmenu/renderwidget';
		$this->data['action_addwidget'] = $this->url->link('module/pavverticalmenu/addwidget', 'token=' . $this->session->data['token'], 'SSL'); 


		$this->data['ajxgenmenu'] 	   = $this->url->link('module/pavverticalmenu/ajxgenmenu', 'root=1&token=' . $this->session->data['token'], 'SSL'); 
		$this->data['ajxmenuinfo'] 	   = $this->url->link('module/pavverticalmenu/ajxmenuinfo', 'root=1&token=' . $this->session->data['token'], 'SSL'); 
	...
	public function ajxgenmenu( ){ 
		...
		if( isset($this->request->post['reset']) ){
	 	...
	 	$this->data['treemenu'] = $this->model_verticalmenu_tree->getTree( 1, true, $params );
